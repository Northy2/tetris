...
const updateInnerFrame = () =>
  drawMatrix(INNER_FRAME_X, INNER_FRAME_Y, INNER_FRAME);

const drawShape = ({ x, y, width, height, shape }) => {
  for (let rowIndex = 0; rowIndex < INNER_FRAME.length; rowIndex++) {
    for (
      let columnIndex = 0;
      columnIndex < INNER_FRAME[rowIndex].length;
      columnIndex++
    ) {
      if (
        rowIndex >= y &&
        ...
        if (shape[rowIndex - y][columnIndex - x])
          INNER_FRAME[rowIndex][columnIndex] =
            shape[rowIndex - y][columnIndex - x];
      }
    }
  }
  updateInnerFrame();
  ...
  for (let rowIndex = 0; rowIndex < INNER_FRAME.length; rowIndex++) {
    for (
      ...
      if (
        ...
        if (shape[rowIndex - y][columnIndex - x])
          INNER_FRAME[rowIndex][columnIndex] = 0;
      }
    }
  }
  updateInnerFrame();
